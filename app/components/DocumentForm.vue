  ...
  <div class="form-container">
    <div class="form-wrapper">
      ...
      <form @submit.prevent="submit" class="form-content">
        <div
          v-for="(groupedFields, subsectionName) in groupedFieldsBySection"
          :key="subsectionName"
          class="section-container"
          :data-section="subsectionName"
          :style="{ borderLeftColor: getSectionColor(subsectionName) }"
        >
          <div
          ...
          >
            <span
              ...
              :class="{ 'is-open': expandedSections[subsectionName] }"
              >●</span
            >
            <h3 class="section-title">
              {{ getSubsectionLabel(subsectionName) }}
            </h3>
          </div>

          <div v-if="expandedSections[subsectionName]" class="section-content">
            <template
              v-for="(fieldsByGroup, groupName) in groupFieldsByGroup(
                groupedFields,
              )"
              :key="groupName"
            >
              <h4 v-if="groupName !== 'sin-grupo'" class="group-header">
                {{ groupName }}
              </h4>
              <div class="fields-grid">
                <div
                  v-for="field in fieldsByGroup"
                  :key="field.name"
                  class="field-wrapper"
                >
                  ...
                  />
                  <div
                    v-else-if="field.type === 'url' && field.preview"
                    class="url-preview-wrapper"
                  >
                    <input
                      v-model="formData[field.name]"
                      type="url"
                  ...
                  <div v-else-if="field.type === 'file'" class="file-wrapper">
                    ...
                    <div v-if="formData[field.name]" class="file-preview">
                      ...
                      <p v-else class="file-preview-name">
                        {{ extractFileName(formData[field.name]) }}
                      </p>
                    </div>
                  </div>
                </div>
...
const toggleSection = (sectionLetter) => {
  expandedSections.value[sectionLetter] =
    !expandedSections.value[sectionLetter];
};
...
const groupFieldsByGroup = (fields) => {
  ...
  fields.forEach((field) => {
    const group = field.group || "sin-grupo";
    if (!grouped[group]) {
      grouped[group] = [];
    }
...
const filterEditableFields = () => {
  ...
  return props.fields.filter((field) =>
    props.editableFieldNames.includes(field.name),
  );
};
...
watch(
  () => Object.keys(groupedFieldsBySection.value),
  (sections) => {
...
const submit = () => {
  ...
  const filteredData = Object.fromEntries(
    Object.entries(formData.value).filter(
      ([_, value]) =>
