...
const equipos = computed(() => (tipo.value ? equipmentStore.obtenerEquipos(tipo.value) : []) || [])
...
const guardarEquipo = async () => {
  ...
  try {
    await equipmentStore.agregarEquipo(tipo.value as string, { ...formulario.value })
    limpiarFormulario()
  ...
  <div class="pagina-equipos">
    ...
    <div v-else-if="config" class="content">
      <div class="contenedor-formulario">
        <h2>➕ Nuevo {{ config.label.slice(0, -1) }}</h2>
      ...
      <div class="contenedor-equipos">
        ...
        <div v-if="equipos.length === 0" class="sin-equipos"><p>📭 No hay {{ config.label.toLowerCase() }} registrados aún</p></div>
        ...
        <button v-if="equipos.length > 0" @click="limpiarLocalStorage" class="btn-limpiar-storage">🔄 Restaurar datos</button>
      </div>
    </div>
  </div>
</template>
