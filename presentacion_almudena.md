...
- **Lección Aprendida sobre Anexos y Casillas de Verificación (T18/T16):** Queda estrictamente establecido que **no se deben inventar ni intentar adivinar selectores** basándose en suposiciones visuales. El script de automatización (`registroService.js`) debe ser un reflejo exacto y 1:1 del script de pruebas exitoso (`test_almudena.js`). 
  - La selección del país en T3 requiere forzar el ID `t5_selec_pais_promotora`.
  - La selección de HULC requiere marcar siempre primero el checkbox `t10_check_doc_reconocido`.
  - Las casillas de documentación adjunta (T18) se marcan exclusivamente usando los IDs comprobados (`t18_check_xml`, `t18_check_cert_firmado`, `t18_check_fic_comprimido`, `t18_check_doc_recomendaciones`, `t18_check_otros_documentos_seccion1`, `t18_check_just_pago`), sin agregar lógica heurística por texto.
- **Estado Actual:** Fases 1 a 4 completadas. En iteración continua de la Fase 5 (Pruebas y Pulido) para asegurar un 100% de éxito en la subida de todos los documentos y paso automático a firma.
